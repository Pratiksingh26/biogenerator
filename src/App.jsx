...
function App() {
  const [name, setName] = useState("Pratik Singh");
  const [location, setLocation] = useState("Nagpur");
  const [stream, setStream] = useState("Engineering");
  const [school, setSchool] = useState("STMES");
  ...
  const generateRandomSchool=()=>{
    let schoolArray=["STVPCET","Janata School","Somayya Polytechnic","STMES"]
    setSchool(schoolArray[Math.floor(Math.random()*4)])
    console.log()

  }
  const generateRandomName=()=>{
    let schoolArray=["Pratik","Ayush","Ankit","Suresh"]
